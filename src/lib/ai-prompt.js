...
11. **block_interaction_type**: Alleen voor blokken_bouwsel — detecteer dit VISUEEL uit de originele opdrachttekst. Kies uit precies vier types:
    ...
    - **"goedFout"**: De leerling ziet ÉÉN 3D-bouwsel én de bijbehorende plattegrond met getallen, en beoordeelt of het bouwsel klopt: twee knoppen "Goed" of "Fout". Gebruik dit als de opdracht vraagt "Klopt dit?", "Is dit goed?", of wanneer een bouwsel vergeleken wordt met een plattegrond. KRITIEK: dit is GEEN meerkeuze met twee bouwsels — er is maar één bouwsel zichtbaar. Vul ALLEEN block_option_a_grid in (het getoonde bouwsel); laat block_option_b_grid LEEG (null).
...
- **Meerkeuze**: maak altijd precies twee VERSCHILLENDE 3D-opties in block_option_a_grid en block_option_b_grid (gebruik nooit dezelfde grid). De foute optie moet plausibel maar duidelijk anders zijn (bijv. één vakje hoger of lager).
- **GoedFout**: vul ALLEEN block_option_a_grid in als het ene te beoordelen bouwsel. Laat block_option_b_grid LEEG (null). Stel block_correct_option in op "A" als het getoonde bouwsel KLOPT met de plattegrond (goed antwoord = "Goed"), of op "B" als het NIET klopt (goed antwoord = "Fout"). Genereer NOOIT een B-grid voor goedFout.
- **Tellen**: vul block_goal_grid in als het te tellen bouwsel. Laat block_option_a_grid en block_option_b_grid LEEG (null). block_correct_option is niet relevant.
- **Bouwen**: vul block_goal_grid én block_plan_grid in. Laat block_option_a_grid en block_option_b_grid LEEG (null). block_correct_option is niet relevant.
